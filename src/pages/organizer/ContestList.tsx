...
const ContestCard: React.FC<ContestCardProps> = ({ contest, onManageProblems, onEditContest, onDeleteContest, index }) => {
    ...
    return (
        <div
            key={contest.id}
            className="bg-theme-secondary border border-theme rounded-xl overflow-hidden transition-all duration-300 hover:border-theme-accent hover:shadow-xl hover:shadow-theme-primary/10 animate-fade-in-slide-up flex flex-col"
            style={{ animationDelay: `${0.1 * index}s` }}
        >
            <div className="p-4 border-b border-theme/50 flex-grow">
                <h2 className="text-lg font-bold text-theme-primary mb-1 line-clamp-2">{contest.title}</h2>
                <p className="text-theme-secondary line-clamp-3 text-xs">{contest.description}</p>
            </div>

            <div className="p-4 grid grid-cols-3 gap-2 text-center border-b border-theme/50">
                <div className="flex flex-col items-center p-1 bg-theme-primary/10 rounded-md">
                    <ListChecks size={18} className="text-purple-400 mb-0.5" />
                    <span className="text-sm font-bold text-theme-primary leading-none">{problemCount}</span>
                    <span className="text-[10px] text-theme-secondary uppercase">Problems</span>
                </div>
                <div className="flex flex-col items-center p-1 bg-theme-primary/10 rounded-md">
                    <Calendar size={18} className="text-green-400 mb-0.5" />
                    <span className="text-xs font-medium text-theme-primary leading-none">
                        {new Date(contest.startTime).toLocaleDateString()}
                    </span>
                    <span className="text-[10px] text-theme-secondary uppercase">Date</span>
                </div>
                <div className="flex flex-col items-center p-1 bg-theme-primary/10 rounded-md">
                    <Clock size={18} className="text-orange-400 mb-0.5" />
                    <span className="text-xs font-medium text-theme-primary leading-none">
                        {new Date(contest.startTime).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})}
                    </span>
                    <span className="text-[10px] text-theme-secondary uppercase">Time</span>
                </div>
            </div>
    ...
    return (
      <div className="min-h-screen bg-theme-primary flex items-center justify-center">
        <div className="text-center animate-bounce-in">
          <div className="animate-spin rounded-full h-16 w-16 border-b-4 border-theme-accent mx-auto mb-4"></div>
          <p className="text-theme-secondary text-base">Loading dashboard data...</p>
        </div>
      </div>
    );
  }
                        ...
                        >
                            <div className="flex-1 pr-3">
                                ...
                                <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${
                                    cp.problem.difficulty === 'EASY' ? 'bg-green-500/20 text-green-400' :
    ...
    <div className="min-h-screen bg-theme-primary p-6 md:p-10">
      <div className="max-w-7xl mx-auto">
        ...
        {showEditContestModal && editingContest && (
          <div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center z-50 p-4 animate-fade-in-slide-up">
            <div className="bg-theme-secondary border border-theme rounded-xl p-6 max-w-2xl w-full max-h-[90vh] overflow-y-auto relative shadow-xl">
              <button
                onClick={() => { setShowEditContestModal(false); setEditingContest(null); }}
                className="absolute top-4 right-4 text-theme-secondary hover:text-theme-primary transition-colors p-2 rounded-full bg-theme-primary/20 hover:bg-theme-primary/30"
              >
                <X size={20} />
              </button>
              ...
              <div className="space-y-4">
                <div>
                  <label className="block text-sm font-medium text-theme-primary mb-2">Contest Title</label>
                  <input
                    type="text"
                    value={editingContest.title}
                    onChange={(e) => setEditingContest({...editingContest, title: e.target.value})}
                    className="w-full px-4 py-2 bg-theme-primary border border-theme rounded-lg text-theme-primary focus:outline-none focus:ring-2 focus:ring-theme-accent"
                  />
                </div>
                ...
                <div>
                  ...
                  <textarea
                    ...
                    className="w-full px-4 py-2 bg-theme-primary border border-theme rounded-lg text-theme-primary focus:outline-none focus:ring-2 focus:ring-theme-accent min-h-[100px]"
                  />
                </div>

                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="block text-sm font-medium text-theme-primary mb-2">Start Time</label>
                    <input
                      type="datetime-local"
                      value={editingContest.startTime.slice(0, 16)}
                  ...
                  <div>
                    <label className="block text-sm font-medium text-theme-primary mb-2">End Time</label>
                    <input
                      type="datetime-local"
                      value={editingContest.endTime.slice(0, 16)}
                ...
                <div className="flex gap-3 mt-6">
                  <button
                    ...
                    className="flex-1 flex items-center justify-center gap-2 bg-green-600/80 hover:bg-green-700 text-white px-4 py-2 rounded-lg transition-colors font-semibold"
                  >
                    <Save size={18} />
                    Save Changes
                  </button>
                  <button
                    onClick={() => { setShowEditContestModal(false); setEditingContest(null); }}
                    className="flex-1 button-theme px-4 py-2 font-semibold"
                  >
                    Cancel
                  </button>
                </div>
